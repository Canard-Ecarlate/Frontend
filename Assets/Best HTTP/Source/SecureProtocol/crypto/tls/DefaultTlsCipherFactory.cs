...
{
    public sealed class DefaultTlsCipherFactory
            ...
            switch (encryptionAlgorithm)
            {
                case EncryptionAlgorithm.cls_3DES_EDE_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: cls_3DES_EDE_CBC", context.UserObject as LoggingContext);
                    return CreateDesEdeCipher(context, macAlgorithm);

                case EncryptionAlgorithm.AES_128_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_128_CBC", context.UserObject as LoggingContext);
                    return CreateAESCipher(context, 16, macAlgorithm);

                case EncryptionAlgorithm.AES_128_CCM:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_128_CCM", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ccm(context, 16, 16);

                case EncryptionAlgorithm.AES_128_CCM_8:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_128_CCM_8", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ccm(context, 16, 8);

                case EncryptionAlgorithm.AES_128_GCM:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_128_GCM", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Gcm(context, 16, 16);

                case EncryptionAlgorithm.AES_128_OCB_TAGLEN96:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_128_OCB_TAGLEN96", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ocb(context, 16, 12);

                case EncryptionAlgorithm.AES_256_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_256_CBC", context.UserObject as LoggingContext);
                    return CreateAESCipher(context, 32, macAlgorithm);

                case EncryptionAlgorithm.AES_256_CCM:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_256_CCM", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ccm(context, 32, 16);

                case EncryptionAlgorithm.AES_256_CCM_8:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_256_CCM_8", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ccm(context, 32, 8);

                case EncryptionAlgorithm.AES_256_GCM:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_256_GCM", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Gcm(context, 32, 16);

                case EncryptionAlgorithm.AES_256_OCB_TAGLEN96:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: AES_256_OCB_TAGLEN96", context.UserObject as LoggingContext);
                    // NOTE: Ignores macAlgorithm
                    return CreateCipher_Aes_Ocb(context, 32, 12);

                case EncryptionAlgorithm.CAMELLIA_128_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: CAMELLIA_128_CBC", context.UserObject as LoggingContext);
                    return CreateCamelliaCipher(context, 16, macAlgorithm);

                case EncryptionAlgorithm.CAMELLIA_128_GCM:
                    // NOTE: Ignores macAlgorithm
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: CAMELLIA_128_GCM", context.UserObject as LoggingContext);
                    return CreateCipher_Camellia_Gcm(context, 16, 16);

                case EncryptionAlgorithm.CAMELLIA_256_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: CAMELLIA_256_CBC", context.UserObject as LoggingContext);
                    return CreateCamelliaCipher(context, 32, macAlgorithm);

                case EncryptionAlgorithm.CAMELLIA_256_GCM:
                    // NOTE: Ignores macAlgorithm
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: CAMELLIA_256_GCM", context.UserObject as LoggingContext);
                    return CreateCipher_Camellia_Gcm(context, 32, 16);

                case EncryptionAlgorithm.CHACHA20_POLY1305:
                    // NOTE: Ignores macAlgorithm
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: CHACHA20_POLY1305", context.UserObject as LoggingContext);
                    return CreateChaCha20Poly1305(context);

                case EncryptionAlgorithm.NULL:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: NULL", context.UserObject as LoggingContext);
                    return CreateNullCipher(context, macAlgorithm);

                case EncryptionAlgorithm.RC4_128:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: RC4_128", context.UserObject as LoggingContext);
                    return CreateRC4Cipher(context, 16, macAlgorithm);

                case EncryptionAlgorithm.SEED_CBC:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: SEED_CBC", context.UserObject as LoggingContext);
                    return CreateSeedCipher(context, macAlgorithm);

                default:
                    if (HTTPManager.Logger.Level <= Loglevels.All)
                        HTTPManager.Logger.Verbose("DefaultTlsCipherFactory", "CreateCipher: Unhandled encryptionAlgorithm: " + encryptionAlgorithm, context.UserObject as LoggingContext);
